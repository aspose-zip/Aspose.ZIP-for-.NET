...
function EnablePasswordProtected(elem) {
	if (elem.checked) {
...
function requestZipFile() {
	let data = fileDrop.prepareFormData();
	if (data === null)
		return;

	let url = o.UIBasePath + 'zip/zip-file?encryptType=' + $('#saveAs').val() + "&setPass=" + $('#chkPasswordProtected').prop("checked") + "&pass=" + $('#txtPassword').val();
